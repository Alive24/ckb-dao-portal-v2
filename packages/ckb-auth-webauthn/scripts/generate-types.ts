...
// Read the generated JSON from moleculec
const jsonPath = path.join(__dirname, '../src/generated/dao.json');
const outputPath = path.join(__dirname, '../src/generated/index.ts');

const json = JSON.parse(fs.readFileSync(jsonPath, 'utf-8'));

// Generate TypeScript types from Molecule schema
let output = `// Auto-generated from dao.mol schema
...
// Helper to convert Molecule type names to TypeScript
function toTsType(moleculeType: string): string {
  switch (moleculeType) {
    case 'byte':
      return 'number';
    case 'Uint8':
...
// Generate interfaces for the types we need
...
output += `// WebAuthn Credential type
...
console.log('Generated TypeScript types from Molecule schema');